...
def convert(mat_path):
    ...
    for i in tqdm(range(netparams.size)):
        ...
        if match:
            ...
            params['backbone.corr_bias'] = torch.from_numpy(squeeze(value))
            continue
        ...
        if layer in [1, 2, 3, 4, 5]:
            ...
            if op == 'conv':  # convolution
                if types == 'f':
                    params['backbone.features.{}.weight'.format(idx)] = torch.from_numpy(value.transpose(3, 2, 0, 1))
                elif types == 'b':#  and layer == 5:
                    value = squeeze(value)
                    params['backbone.features.{}.bias'.format(idx)] = torch.from_numpy(value)
            elif op == 'bn':  # batch normalization
                if types == 'x':
                    m, v = squeeze(np.split(value, 2, 1))
                    params['backbone.features.{}.running_mean'.format(idx)] = torch.from_numpy(m)
                    params['backbone.features.{}.running_var'.format(idx)] = torch.from_numpy(np.square(v))
                    # params['features.{}.num_batches_tracked'.format(idx)] = torch.zeros(0)
                elif types == 'm':
                    value = squeeze(value)
                    params['backbone.features.{}.weight'.format(idx)] = torch.from_numpy(value)
                elif types == 'b':
                    value = squeeze(value)
                    params['backbone.features.{}.bias'.format(idx)] = torch.from_numpy(value)
            else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mat_path', type=str, default="./2016-08-17_gray025.net.mat")
